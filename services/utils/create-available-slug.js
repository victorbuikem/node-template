...
const findActiveCardBySlug = require('./find-active-card-by-slug');
...
async function createAvailableSuffixedSlug(suffixBase) {
  ...
  const existing = await findActiveCardBySlug(suffixedSlug);
...
async function createAvailableSlug(title) {
  ...
  const existing = slug.length >= SLUG_MIN_LENGTH ? await findActiveCardBySlug(slug) : null;
